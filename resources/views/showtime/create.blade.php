<x-app-layout>
    ...
    <div class="bg-white rounded-xl shadow-lg p-6 mb-8">
        <form action="{{ route('showtime.store') }}" method="POST" class="space-y-6">
            ...
            <!-- Film -->
            <div>
                <label for="movie_id" class="block text-sm font-medium text-gray-700 mb-1">Film</label>
                <select name="movie_id" id="movie_id" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
            ...
            <div>
                ...
                <select name="room_id" id="room_id" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                    <option value="">Sélectionnez une salle</option>
                    @foreach($rooms as $room)
                        <option value="{{ $room->id }}" {{ old('room_id') == $room->id ? 'selected' : '' }}>
                            {{ $room->name }} - {{ $room->cinema->name }} ({{ $room->capacity }} places)
                        </option>
                    @endforeach
                </select>
                @error('room_id')
                ...
                <div class="flex">
                    ...
                    <div class="ml-3">
                        ...
                        <div class="mt-2 text-sm text-blue-700">
                            <p>Assurez-vous que la salle est disponible pour la date et l'heure choisies.</p>
                        </div>
                    </div>
                </div>
            </div>
