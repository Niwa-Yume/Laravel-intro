<x-app-layout>
    ...
    <div class="bg-white rounded-xl shadow-lg p-6 mb-8">
        <form action="{{ route('showtime.update', $showtime->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
                ...
                <select name="room_id" id="room_id"
                    ...
                    @foreach($rooms as $room)
                        <option value="{{ $room->id }}" {{ old('room_id', $showtime->room_id) == $room->id ? 'selected' : '' }}>
                            {{ $room->cinema->name }} - {{ $room->name }} ({{ $room->capacity }} places)
                        </option>
                    @endforeach
                </select>
                @error('room_id')
            ...
            <div>
                ...
                <input type="datetime-local" name="start_time" id="start_time"
                       value="{{ old('start_time', date('Y-m-d\TH:i', strtotime($showtime->start_time))) }}"
                       class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                       required>
                @error('start_time')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div class="flex">
                    ...
                    <div class="ml-3">
                        ...
                        <div class="mt-2 text-sm text-blue-700">
                            <p>Assurez-vous que la salle est disponible à la date et l'heure choisies.</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="mt-4 border-t border-gray-200 pt-4">
            <form action="{{ route('showtime.destroy', $showtime->id) }}" method="POST" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer cette séance?');">
                @csrf
                @method('DELETE')
